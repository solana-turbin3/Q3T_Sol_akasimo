...
(async () => {
    const MY_WALLET_ADDRESS = "HJJGipfRiFz6S3g4RFfkZHLUyyLULb7QiH4Anwisd4qi";
    ...
    accounts.forEach((account, i) => {
        ...
        if (!account.account ) {
            console.log("No token account data found");
            return;
        }

        if (!('parsed' in account.account.data)) {
            console.log("No parsed token account data found");
            return;
        }
